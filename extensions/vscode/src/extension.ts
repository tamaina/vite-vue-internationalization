...
import { createRefreshQueue } from './refreshQueue.js';
...
export async function activate(context: vscode.ExtensionContext) {
	...
	const refreshQueue = createRefreshQueue(prepareRefresh);
	const schedule = () => {
	...
	async function refreshSafely() {
		...
		try { await refreshQueue.refresh(); } catch (error) {
	...
	async function prepareRefresh() {
		...
		const buffers = new Map(vscode.workspace.textDocuments.filter(document => document.uri.scheme === 'file' && document.isDirty).map(document => [resolve(document.uri.fsPath), document.getText()]));
	...
	context.subscriptions.push(collection, output, configs, vscode.commands.registerCommand('vvi.refreshDiagnostics', refreshSafely),
		...
		{ dispose() { refreshQueue.dispose(); if (timer) clearTimeout(timer); for (const watcher of watchers.values()) watcher.dispose(); } });
	await refreshSafely();
