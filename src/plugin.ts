...
export function vueInternationalization(options?: Partial<VueInternationalizationOptions>): Plugin {
	...
	return {
		...
		augmentChunkHash() {
			...
			state.localeHash ??= createLocaleHash(['vvi-inline-output-v7', modules, globalMessages, currentOptions.primaryLocale, currentOptions.messageSyntax, this.environment.config.build.sourcemap]);
